...
const page = (ok, back, why) => new Response('<!DOCTYPE html><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="robots" content="noindex"><title>' + (ok ? 'Message sent' : 'Not sent') + '</title><div style="font-family:system-ui,sans-serif;max-width:32rem;margin:15vh auto;padding:1rem;text-align:center"><h1 style="font-size:1.6rem">' + (ok ? 'Thanks — your message was sent.' : 'Sorry, that did not go through.') + '</h1><p style="color:#555">' + (ok ? 'We will get back to you soon.' : (why || 'Please go back and try again.')) + '</p><p><a href="' + back + '">Back to the page</a></p></div>', { status: ok ? 200 : 400, headers: { 'content-type': 'text/html; charset=utf-8', 'x-robots-tag': 'noindex' } });
export async function onRequestPost({ request, env }) {
  ...
  let f;
  try { f = await request.formData(); } catch (e) { return page(false, back, 'The form could not be read. Please go back and try again.'); }
